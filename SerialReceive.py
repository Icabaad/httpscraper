...
#Logging addeed 16-05-2017
logger = logging.getLogger('myapp')
hdlr = logging.FileHandler('/usr/local/sbin/log.txt')
...
while True:
    ...
    sleep(2.0)
    if len(data) >= 2 and data[0] == '{' and data.find('}'):
        parsed = data.split(",")
        datalist = data.split(',')
...
#        logger.info(parsed)
        ...
        if len(datalist) == 10:
            ...
            try:
                ...
                print "Error: %s" % ex
                logger.error('****DangerProxy Power upload error****')
            ...
            try:
                ...
                print "Error: %s" % ex
                logger.error('****DangerProxy Minute upload error****')

        else:
            print 'XXXXXRogue Serial String recieved!XXXXX'
            logger.error('****ROGUE string received****')
            logger.error(data)
            print datalist
            errorcount += 1
            print 'Error Count: ', errorcount
